...
export class Hud {
  ...
  private braceText!: HTMLElement; private braceBar!: HTMLElement; private hazard!: HTMLElement; private hazardText!: HTMLElement; private hazardBar!: HTMLElement;
  private radioT = 0;
  constructor(parent: HTMLElement) {
    ...
    this.prompt = el('div', 'prompt', this.root); this.msg = el('div', 'msg', this.root); this.brace = el('div', 'brace', this.root, `<span>${t('brace')}</span><div class="bar"><i></i></div>`); this.braceText = this.brace.querySelector('span')!; this.braceBar = this.brace.querySelector('i')!;
    this.hazard = el('div', 'hazard', this.root, `<span></span><div class="bar"><i></i></div>`); this.hazardText = this.hazard.querySelector('span')!; this.hazardBar = this.hazard.querySelector('i')!; this.hazard.hidden = true;
  ...
  relabel() { this.alt.innerHTML = `${t('altitude')}<b>0<small>m</small></b>`; this.stam.firstChild!.textContent = t('stamina'); this.wind.firstChild!.textContent = t('wind'); this.braceText.textContent = t('brace'); }
  ...
  update(dt: number, c: Climber, w: Weather, objective: string) {
    ...
    if (c.brace) { const gi = w.gustIn; const f = w.gust > 0 ? w.gust : gi >= 0 ? 1 - gi / Math.max(0.1, w.warnLead) : 0; this.braceBar.style.width = `${clamp(f, 0, 1) * 100}%`; this.braceText.textContent = c.onLadder && c.gripping === 1 ? `${t('brace')} ${t('twoHands')}` : t('brace'); }
    this.hazard.hidden = !(c.onLadder && c.arcWarn > 0); if (c.arcWarn > 0) { const arcing = c.arcWarn >= 1; this.hazard.classList.toggle('on', arcing); this.hazardText.textContent = `⚡ ${arcing ? t('arcNow') : t('arcWarn')}`; this.hazardBar.style.width = `${(arcing ? 1 : c.arcWarn) * 100}%`; }
